...
type FertilityAttribute = { attribute:string; slug:string; mean:number; min:number; max:number; pearson:number|null; spearman:number|null; r2:number|null; map_url:string; scatter_url:string };
    ...
    <>
      <header className="header">
        <div className="header-inner">
          ...
          <div className="badge">Versão 3.1 · Colheita + Fertilidade</div>
        </div>
      </header>
      ...
      <main>
        ...
        {fertility && <section id="fertilidade-resultado" className="card result-card"><h2>4. Resultado da fertilidade</h2><div className="fertility-actions"><a className="secondary pdf-button" href={`${api}${fertility.pdf_url}`}>Baixar relatório completo em PDF</a><a className="secondary outline-button" href={`${api}${fertility.download_url}`}>Baixar arquivos da fertilidade (ZIP)</a></div><div className="nutrient-results">{fertility.attributes.map(a=><article className="nutrient-card" key={a.slug}><h3>{a.attribute}</h3><div className="nutrient-images"><img src={`${api}${a.map_url}`} alt={`Mapa ${a.attribute}`}/><img src={`${api}${a.scatter_url}`} alt={`Correlação ${a.attribute}`}/></div><div className="stats stats-six"><div className="stat"><span>Média</span><b>{formatNumber(a.mean,2)}</b></div><div className="stat"><span>Mínimo</span><b>{formatNumber(a.min,2)}</b></div><div className="stat"><span>Máximo</span><b>{formatNumber(a.max,2)}</b></div><div className="stat"><span>Pearson</span><b>{a.pearson===null?'—':formatNumber(a.pearson,3)}</b></div><div className="stat"><span>Spearman</span><b>{a.spearman===null?'—':formatNumber(a.spearman,3)}</b></div><div className="stat"><span>R²</span><b>{a.r2===null?'—':formatNumber(a.r2,3)}</b></div></div></article>)}</div></section>}
      </main>
    </>
  );
}
