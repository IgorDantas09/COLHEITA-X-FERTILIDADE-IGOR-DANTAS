...
def _plot_nutrient(boundary, samples, attr, raster, transform, outpath: Path):
    ...
    cmap=LinearSegmentedColormap.from_list("fert",["#d7191c","#fdae61","#ffffbf","#a6d96a","#1a9641"],N=max(5,len(bins)-1))
    finite_bins=np.asarray(bins,dtype=float)
    finite_bins[0]=np.nanmin(raster[np.isfinite(raster)])-1e-6 if finite_bins[0]<-1e8 else finite_bins[0]
    finite_bins[-1]=np.nanmax(raster[np.isfinite(raster)])+1e-6 if finite_bins[-1]>1e8 else finite_bins[-1]
...
def process_fertility(excel_path: Path, yield_output: Path, outdir: Path, attrs: list[str], cfg: FertilityConfig) -> dict:
    ...
    summary={"version":"3.0-fertility","attributes":results,"sample_points":len(samples),"buffer_radius_m":cfg.buffer_radius_m,"idw_power":cfg.idw_power,"pixel_size_m":cfg.pixel_size_m,"crs":TARGET_CRS}
