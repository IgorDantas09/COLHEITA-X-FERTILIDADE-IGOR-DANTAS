...
PRESETS = {
    ...
    "Ca [cmolc/dm³]": ([-1e9,1.5,2.5,3.5,4.5,1e9], "Ca", "cmolc/dm³"),
    "Mg [cmolc/dm³]": ([-1e9,0.5,1.0,1.5,2.0,1e9], "Mg", "cmolc/dm³"),
    "K [ppm]": ([-1e9,50,75,100,125,1e9], "K", "ppm"),
    "S [mg/dm³]": ([-1e9,10,15,20,25,1e9], "S", "mg/dm³"),
    "B [mg/dm³]": ([-1e9,0.6,0.9,1.2,1.5,1e9], "B", "mg/dm³"),
...
def _plot_yield_at_samples(boundary: gpd.GeoDataFrame, samples: gpd.GeoDataFrame, yield_tif: Path, unit: str, outpath: Path) -> None:
    """Gera o mapa de colheita reutilizando o raster já processado.

    Os rótulos representam a produtividade média dentro do buffer definido
    ao redor de cada ponto de amostragem de solo. Não há nova interpolação.
    """
    with rasterio.open(yield_tif) as src:
        arr = src.read(1, masked=True)
    ...
    if finite.size == 0:
        raise ValueError("O mapa de produtividade não possui valores válidos.")

    # Mantém a mesma linguagem visual vermelho-amarelo-verde do mapa principal.
    cmap = LinearSegmentedColormap.from_list(
        "yield", ["#d7191c", "#f46d43", "#fdae61", "#ffffbf", "#a6d96a", "#1a9641"], N=256
    )
    vmin, vmax = np.nanpercentile(finite, [2, 98])
    if math.isclose(float(vmin), float(vmax)):
        vmax = float(vmin) + 1e-6

    fig = plt.figure(figsize=(8.2, 6.0), dpi=130, facecolor="white")
    ax = fig.add_axes([0.08, 0.10, 0.78, 0.80])
    cax = fig.add_axes([0.89, 0.18, 0.025, 0.64])
    im = ax.imshow(values, extent=extent, origin="upper", cmap=cmap, vmin=vmin, vmax=vmax)
    boundary.boundary.plot(ax=ax, color="#26352c", linewidth=1.5)
    samples.plot(ax=ax, color="#1554a0", markersize=24, edgecolor="white", linewidth=0.7, zorder=4)

    for _, row in samples.iterrows():
        value = row.get("produtividade_50m")
        if pd.notna(value):
            ax.annotate(
                f"{float(value):.2f}",
                ...
                ha="center", va="bottom", fontsize=7.2, fontweight="bold", color="#17231c",
    ...
    ax.set_title("Mapa de colheita — produtividade média em raio de 50 m", fontsize=12, fontweight="bold", pad=10)
    cb = fig.colorbar(im, cax=cax)
    cb.set_label(unit)
    fig.savefig(outpath, dpi=140, facecolor="white", bbox_inches="tight", pad_inches=0.25)
    plt.close(fig)
...
def process_fertility(excel_path: Path, yield_output: Path, outdir: Path, attrs: list[str], cfg: FertilityConfig) -> dict:
    ...
    yield_points_name = "colheita_pontos_produtividade_50m.png"
    _plot_yield_at_samples(
        boundary, samples, yield_output/"mapa_produtividade.tif",
        yield_summary.get("unit", ""), outdir/yield_points_name
    )
    for attr in final_attrs:
        ...
        results.append({"attribute":attr,"slug":slug,"mean":float(np.nanmean(x)),"min":float(np.nanmin(x)),"max":float(np.nanmax(x)),"pearson":pear,"spearman":spear,"r2":r2,"map_url":f"fertilidade_{slug}.png","yield_map_url":yield_points_name,"scatter_url":f"dispersao_{slug}.png"})
    ...
    summary={"version":"3.2-fertility-yield-points","attributes":results,"sample_points":len(samples),"buffer_radius_m":cfg.buffer_radius_m,"idw_power":cfg.idw_power,"pixel_size_m":cfg.pixel_size_m,"crs":TARGET_CRS}
