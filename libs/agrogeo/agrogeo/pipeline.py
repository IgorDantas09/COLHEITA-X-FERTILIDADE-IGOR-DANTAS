...
def save_outputs(boundary, x, y, values, reason,
    ...
    fig, ax = plt.subplots(figsize=(13.33, 7.5), dpi=110)
    extent = [
        transform.c,
    ...
    cb.set_label(unit)
    info = (
        f"Pontos mantidos: {summary['kept_points']:,}\n"
        f"Pontos removidos: {summary['removed_points']:,} ({summary['removal_pct']:.1f}%)\n"
        f"Krigagem ordinária local: {model}\n"
        f"Pixel: {cfg.pixel_size_m:g} m · EPSG:31981"
    )
    ax.text(
        0.012, 0.018, info, transform=ax.transAxes, fontsize=9, va="bottom",
        bbox=dict(facecolor="white", alpha=0.88, edgecolor="#adb8b0", boxstyle="round,pad=.5")
    )
    fig.tight_layout()
    fig.savefig(outdir / "mapa_produtividade.png", bbox_inches="tight")
    plt.close(fig)
