...
def save_outputs(boundary, x, y, values, reason,
    ...
    # Prévia limpa e centralizada. A margem espacial impede que o limite do talhão
    # fique encostado nos eixos e melhora a composição na folha A4 horizontal.
    fig = plt.figure(figsize=(13.33, 7.5), dpi=120, facecolor="white")
    ax = fig.add_axes([0.12, 0.14, 0.68, 0.72])
    cax = fig.add_axes([0.835, 0.18, 0.022, 0.64])
    extent = [
        transform.c,
    ...
    ax.set_anchor("C")
    ax.set_xlabel("Leste (m)", fontsize=10, labelpad=8)
    ax.set_ylabel("Norte (m)", fontsize=10, labelpad=8)
    ...
    cb.set_label(unit, fontsize=10, labelpad=9)
    cb.ax.tick_params(labelsize=9)
    fig.savefig(outdir / "mapa_produtividade.png", facecolor="white", dpi=130)
    plt.close(fig)
