...
app = FastAPI(title="Colheita × Fertilidade API", version="3.2.0")
app.add_middleware(
    CORSMiddleware,
...
def health():
    cleanup()
    return {"status": "ok", "version": "3.2.0-fertility-yield-points", "temporary_ttl_minutes": TTL // 60}
...
async def process_fertility_job(
    ...
    try:
        ...
        return {"job_id":job_id,"summary":summary,"pdf_url":f"/jobs/{job_id}/fertility/pdf","download_url":f"/jobs/{job_id}/fertility/download","attributes":[{**a,"map_url":f"/jobs/{job_id}/fertility/files/{a['map_url']}","yield_map_url":f"/jobs/{job_id}/fertility/files/{a['yield_map_url']}","scatter_url":f"/jobs/{job_id}/fertility/files/{a['scatter_url']}"} for a in summary['attributes']]}
